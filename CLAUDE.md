...
**`r"…"` is one regex, everywhere: the compile-time-compiled, linear-time DFA** (decided 08/28 — the RE2 discipline; there is deliberately no second engine, and E054's refusals are properties of the language's regex in every position). Two positions consume the same tables today: a constraint's argument, and a **`match` pattern on a string scrutinee** — `match s { r"^#[0-9a-fA-F]{6}$" => …, w @ r"^[a-z]+$" => …, _ => … }` — where each arm is one call to the shared driver, an invalid or un-table-able pattern is a compile error at the arm, and regex arms never make a match exhaustive. As a *value* (`let re = r"[a-z]+"`) it is still `lyra-E052`: a future `Regex` type will be literal-constructed and compile-time-compiled too; runtime pattern construction from strings would be a separate deliberate feature, not a widening of `r"…"`. There is no `regex` type to annotate either — a lowercase type name parses as a type variable, so `(re: regex)` declares one called `regex`.

A newtype is **transparent to its base's methods** — `newtype Name = string` supports `len`/`slice`/`trim`, builtins and prelude `self:` functions alike. The fallback is tried after every other rung, so a method written for the newtype still wins. **Calls are the same transparency for a function-type base** (08/28): `h(5)` on a `newtype Handler = (i64) -> i64` value dispatches through the base's signature in every position — binding, parameter, struct field — exactly as indexing looks through a newtype over an array; without it a function-type newtype was write-only in practice.
...
## A Generic Declared Inside a Function
...
inside a generic function — or another local generic — whose variables its signature and body
may mention, at any depth. A lambda
