...
- **`pkg/analyzer/checker`** — standalone AST passes (use-before-declaration, purity/effect bounds, recursive types, lints); since 07/21 also **`use_after_move.go`** (`lyra-E019`), a flow-sensitive definite-move analysis flagging a read of a binding after it was moved into an `own` parameter (union at branch joins, loop bodies seeded with their own moves, declaration/reassignment clears); and **`inert_borrow_modifier.go`** (`lyra-W010`), warning when `own`/`ref`/`mut` sits on a copied scalar primitive where it has no effect; and **`range_analysis.go`** (`lyra-E020`/`W011`), a flow-sensitive value-range (interval) analysis tracking each integer variable's `[lo,hi]` to catch a *definite* overflow on a non-constant variable (`if x > 100 { x + 100 }` on an i8 — via branch refinement, which constant-folding can't see) and an always-true/false comparison; zero-false-positive (anything imprecise widens to the type's full range), diagnostics-only for now (trap *elision* is the deferred optimizer half)
